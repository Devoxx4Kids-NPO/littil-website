...
if (!function_exists('oc_validate_domain')) {
    function oc_validate_domain($force = false, $domain = null)
    {
        ...
        if (!empty($oc_validate_domain) && false === $force) {
...
if (!function_exists('oc_validate_action_cb')) {
    ...
    {
        $data = isset($_POST['operation']) ? filter_var($_POST['operation'], FILTER_SANITIZE_STRING) : '';
        $action_type = isset($_POST['actionType']) ? filter_var($_POST['actionType'], FILTER_SANITIZE_STRING) : '';
        ...
        } else if (oc_pm_features($data, $result[$data_str]) || in_array('MWP_ADDON', $result[$data_str])) {
            $response = [
    ...
    function onecom_upgrade_modal()
    {
            ...
            <div id="oc_um_wrapper">
                <div class="oc-bg-white">
                    ...
                    <div id="oc_um_body">
                        ...
                        echo '<ul>' . '<li>' . __('Quick fix or ignore recommendations', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Get better performance with Performance Cache and CDN', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Get notified about security with Vulnerability Monitoring', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Get helpful tips with Advanced Error Page', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Get access to our Premium themes', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Increase your authentication security', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '<li>' . __('Host on our WordPress servers built for speed', OC_VALIDATOR_DOMAIN) . '</li>';
                        echo '</ul>';
                        ?>
                    </div>
                ...
                <?php /* @todo remove this after review <div id="oc_um_close"
                 * onclick="jQuery('#oc_um_overlay').hide();jQuery('.loading-overlay.fullscreen-loader').removeClass('show');"></div> */ ?>
            </div>
        ...
        <style>
            ...
            #oc_um_body ul {
                ...
                list-style-image: url("data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTYiIGhlaWdodD0iMTMiIHZpZXdCb3g9IjAgMCAxNiAxMyIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KICAgIDxwYXRoIGQ9Ik0wLjc1IDUuNzVMNS43NSAxMC43NUwxNS4yNSAxLjI1IiBzdHJva2U9IiM3NkI4MkEiIHN0cm9rZS13aWR0aD0iMiIvPgo8L3N2Zz4K");
            }

            .oc_up_btn:hover, .oc_um_btn:hover {
                /*opacity: 0.6;*/
            }
        </style>
        <?php
...
if (!function_exists('onecom_hide_plugins')) {
    function onecom_hide_plugins($plugins)
    {
        ...
        foreach ($wp_list_table->items as $key => $plugin) {
            ...
            if ($url_condition && in_array($plugin['Author'], ['onecom', 'one.com'])) {
...
function onecom_autoupdates($update, $item)
{
    if (
        ...
        && in_array($item->Author, ['one.com', 'onecom'])
    ) {
...
function onecom_automatic_updates_complete( $update_results ) {
	...
	foreach ( $update_results['plugin'] as $result ) {
	    if ( property_exists($result, 'item')
            ...
            && in_array($result->item->Author, ['one.com', 'onecom'])
        ) {
...
// Function to get UTM medium (current page tracking) for CP links landing
if (!function_exists('oc_utm_medium')) {
    function oc_utm_medium() {
        
        if (isset($_GET['page']) && 
            $_GET['page'] == 'onecom-wp-health-monitor') {
                return $utm_medium = 'health_monitor';
        } else if (isset($_GET['page']) &&
            $_GET['page'] == 'onecom-wp-vulnerability-monitor') {
                return $utm_medium = 'vulnerability_monitor';
        } else if (isset($_GET['page']) &&
            $_GET['page'] == 'onecom-wp-under-construction') {
                return $utm_medium = 'maintenance_mode';
        } else if (isset($_GET['page']) &&
            $_GET['page'] == 'onecom-vcache-plugin') {
                return $utm_medium = 'performance_cache';
        } else if (isset($_GET['page']) &&
            $_GET['page'] == 'onecom-wp-spam-protection') {
                return $utm_medium = 'spam_protection';   
        } else if (isset($_GET['page']) &&
            $_GET['page'] == 'onecom-wp-staging') {
                return $utm_medium = 'staging';   
        } else if (isset($_GET['page'])) {
            $current_page = str_replace('-', '_', $_GET['page']);
            return $utm_medium = $current_page;
        } else {
            // Ideally, there should not be any current case
            return $utm_medium = '';
        }
    }
}
...
if (!function_exists('oc_upgrade_link')) {
    ...
    {
        // Set UTM medium as a combination of current page + link type/location
        if (isset($type) && !empty($type)) {
            $utm_medium_location = oc_utm_medium().'_'.$type;
        } else {
            $utm_medium_location = oc_utm_medium();
        }
        ...
        $utm_param = "&utm_source=onecom_wp_plugin&utm_medium=".$utm_medium_location;

        return $first_part . $middle_part . $last_part . $utm_param;
