...
;(function($) {
  OT_UI = {
    ...
    init_social_grid:function(){
        if(true || jQuery('img.social_icons_array').length){
          jQuery(document).on('click', 'img.social_icons_array', function(){
                jQuery(this).parent().parent().parent().parent().parent().find('input.option-tree-setting-title').val(jQuery(this).attr('alt'));
                jQuery(this).parent().parent().parent().parent().parent().parent().find('.open').html(jQuery(this).attr('alt'));
            });
        }
    },
    init_add: function() {
    ...
    parse_condition: function() {
      $( '.format-settings[id^="setting_"][data-condition], .ot-theme-option-nav li[data-condition], .ot-theme-option-nav[data-condition]' ).each(function() {
    ...
    init_upload: function() {
      $(document).on('click', '.ot_upload_media', function() {
        var parent_id           = $(this).parent('.option-tree-ui-upload-parent');
        ...
        if ( window.wp && wp.media ) {
          ...
          window.ot_media_frame.on('select', function() {
            ...
            removeBtn = '<a href="javascript:(void);" class="ot_remove_media option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon ot-icon-minus-circle"></span>'+option_tree.remove_media_text+'</a>';
            ...
            if(! $(parent_id).find('.ot_remove_media').length){ $(parent_id).append(removeBtn); }
    ...
    init_upload_remove: function() {
      $(document).on('click', '.option-tree-ui-remove-media', function(event) {
        ...
        if (agree) {
          ...
          $(this).parent().siblings('.option-tree-ui-media-wrap').remove();
          $(this).remove();
          //$(this).parent().parent().find('.option-tree-ui-media-wrap').remove();
          //$(this).parent('.format-setting-inner').remove();
          return false;
        }
        return false;
      });
    ...
    init_tabs: function() {
      ...
      // save active tabs data into localStorage of browser
      $(".wrap.settings-wrap .ui-tabs a.ui-tabs-anchor").on("click", function(event, ui) {

        var onecomThmActTab={
          theme : option_tree.active_theme,
          main : $(this).attr('id'),
          type : 'toplevel_page_octheme_settings'
        };
        localStorage.setItem('onecomThmActTab', JSON.stringify(onecomThmActTab));
        var targetTabContent = $(this).attr('href');
        var subTabAnchors = $(targetTabContent).find(".ui-tabs a.ui-tabs-anchor");
        if(subTabAnchors.length){
            $(subTabAnchors).on("click", function() {
              onecomThmActTab.sub =  $(this).attr('id');
              localStorage.setItem('onecomThmActTab', JSON.stringify(onecomThmActTab));
            });
    ...
    last_active_tabs: function(){
        var lastActive = localStorage.getItem('onecomThmActTab');
        var tabs = JSON.parse(lastActive) || {};
        if(
            tabs.hasOwnProperty('main') &&
            tabs.hasOwnProperty('theme') &&
            tabs.theme === option_tree.active_theme &&
            $('body').hasClass(tabs.type)
        ){
          var mainTab = $('#'+tabs['main']).selector;

          setTimeout(function(){
              if($(mainTab).length && $(mainTab).parent().attr('aria-selected') == 'false' ){
                  $(mainTab).trigger('click');
              }
          }, 400);

          if(tabs.hasOwnProperty('sub')){
            var subTab = $('#'+tabs['sub']).selector;
            setTimeout(function(){
                if($(subTab).length && $(subTab).parent().attr('aria-selected') == 'false'){
      ...
      this._frame.on('update', function () {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            /* Add BG image url to <li> inside "res"
             * More details : https://app.asana.com/0/307895785186248/511799093206146/f
             */

            var new_res='';
            jQuery(res).each(function (i,v) {
                new_res += '<li style="background-image:url('+jQuery(v).find('img').attr('src')+')">'+v.innerHTML+'</li>';

            });

            parent.children('.ot-gallery-list').html(new_res);
        ...
        $(this).find('.ot-metabox-tabs').tabs({
          activate: function( event, ui ) {
            var parent = $(this).outerHeight(),
                ...
                minHeight = parent - 34,
                tabLinks = $(this).find('.ui-state-active');


            var onecomThmActTab={
                theme : option_tree.active_theme,
                main : $(tabLinks).find('a.ui-tabs-anchor').attr('id'),
                type : 'post-php'
            };
            localStorage.setItem('onecomThmActTab', JSON.stringify(onecomThmActTab));
      ...
      if ( $(this).find('.type-tab').length ) {
        ...
        $(this).find('.format-settings').each( function() {
          ...
          if ( $(this).find('.type-tab').length > 0 ) {
            ...
            /* Code Update : Added condition variable to carry switch-condition in above statement */

            // Add a class, hide & append nav item
              $(this).addClass('is-panel').hide();
              if(condition){
                  $(this).parents('.ot-theme-option-panels').prev('.ot-theme-option-nav').append('<li data-condition="'+condition+'"><a href="#' + id + '">' + title + '</a></li>');
                /* Code Update : Applied condition on <tab> in above statement  */
                  $(this).parents('.ot-theme-option-panels').prev('.ot-theme-option-nav').attr('data-condition', condition);
              }
              else{
                  $(this).parents('.ot-theme-option-panels').prev('.ot-theme-option-nav').append('<li><a href="#' + id + '">' + title + '</a></li>')
              }
              $(this).addClass('is-panel').removeAttr('data-condition');
             /* Code Update : Removed condition from hidden tab-title so that it can be hidden always intentionally  */
